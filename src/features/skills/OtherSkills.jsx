...
const OtherSkills = () => {
  return (
    <div className="flex flex-col gap-14 items-center justify-center sm:px-8 md:px-16">
      <ul className="flex flex-wrap justify-center gap-3 sm:gap-4 md:gap-6 uppercase tracking-wide">
        {navLists.map((list, index) => (
          <li key={index} className="font-serif text-sm">
            <span className="bg-blue-700 text-white border border-blue-700 px-4 py-2 rounded-full shadow-[0_0_10px_2px_rgba(37,99,235,0.3)] hover:shadow-[0_0_15px_3px_rgba(37,99,235,0.5)] hover:scale-105 hover:translate-x-1 transition-transform duration-300 ease-in-out">
              {list.label}
            </span>
          </li>
        ))}
